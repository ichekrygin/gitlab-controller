...
func TestGitLabSetFailed(t *testing.T) {
	for _, tt := range setTestCases(failed, pending) {
		t.Run(tt.name, func(t *testing.T) {
...
func TestGitLabSetPending(t *testing.T) {
	for _, tt := range setTestCases(pending, failed) {
		t.Run(tt.name, func(t *testing.T) {
...
func TestGitLab_GetProviderName(t *testing.T) {
	tests := map[string]struct {
		spec GitLabSpec
		want string
	}{
		"Default": {spec: GitLabSpec{}, want: ""},
		"Empty":   {spec: GitLabSpec{ProviderRef: corev1.ObjectReference{}}, want: ""},
		"Custom":  {spec: GitLabSpec{ProviderRef: corev1.ObjectReference{Name: "foo", Namespace: "bar"}}, want: "foo"},
	}
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			gl := &GitLab{
			...
			if got := gl.GetProviderName(); got != tt.want {
				t.Errorf("GitLab.GetProviderName() = %v, want %v", got, tt.want)
			}
		})
	}
}
